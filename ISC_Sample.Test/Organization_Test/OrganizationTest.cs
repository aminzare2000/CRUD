...
public class OrganizationTest
{
    ...
    public void Create_Read_Update_Delete_Organization()
    {
        using (var _dbContext = new IscDbContext(configuration: _configuration))
        {
            ...
            #region Update _Organization_List_Test
            ...
            var getOrganizationAfterUpdate = _dbContext.Organizations.ToList();
            Assert.IsTrue(getOrganizationAfterUpdate.Any(it => it.OrganizationName == "ISCOrganization"));
            Assert.IsTrue(getOrganizationAfterUpdate.Any(it => it.Depart1 == "Adabiat"));
            Assert.IsTrue(getOrganizationAfterUpdate.Any(it => it.Depart2 == "Persian"));
            Assert.IsTrue(getOrganizationAfterUpdate.Any(it => it.Depart3 == "FarhanghLoghat"));
